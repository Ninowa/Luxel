    ...
    public class MotionController : Controller
    {
        ...
        public void Update()
        {
           ...
           SetAnimatorProperties(mState, lUseTrendData);
            ...
            if (_UseInput)
            {
                Log.ScreenWrite(String.Format("AC.Update() Time:{0:f4}ms {1} Mx:{2:f4} My:{3:f4} Vx:{4:f4} Vy:{5:f4} MCTime:{6:f4}ms", Time.deltaTime, CurrentMotionName, ootiiInputStub.MovementX, ootiiInputStub.MovementY, ootiiInputStub.ViewX, ootiiInputStub.ViewY, mUpdateProfiler.Time), 2);
